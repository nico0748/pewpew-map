...
export const supplyChain: AttackDefinition = {
  meta,
  appliesIf: [
    { head: 'npm/pip/Maven 等の依存マネージャを使っている:', body: 'OSS ライブラリを使う全ての現代的アプリが対象。直接依存だけでなく推移的依存も含む。' },
    { head: 'GitHub Actions など第三者のCI Actionを使う:', body: 'タグ参照(@v3)はあとから差し替え可能。コミットSHAでpinしないと汚染を取り込みうる。' },
    { head: 'Docker/コンテナイメージをパブリックに依存:', body: '上流イメージのバックドア混入を取り込む経路。' },
    { head: 'パッケージを配布している側:', body: 'CI/CD・署名鍵・公開レジストリの管理が攻撃面。SolarWinds型の被害発信源になりうる。' },
    { head: 'SBOMが整備されていない:', body: '脆弱性発生時に影響箇所を即特定できないと、対応に致命的な遅れが出る。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: 'npm / pip / Maven などのライブラリ管理を使っている:', body: '世の中のライブラリを自動で取り込む仕組み(=ほぼすべての現代的アプリ)が対象です。' },
      { head: 'GitHub Actions などのCIで第三者の Action を使っている:', body: '`@v3` のようなタグ参照は後から差し替え可能。コミットSHAで固定しないと汚染を取り込みかねません。' },
      { head: 'Docker などのコンテナイメージを上流から取得:', body: '元になっているイメージにバックドアが混入していると、それを継承して使ってしまいます。' },
      { head: '自社が配布物(ライブラリ・SaaS・アプリ)を提供している:', body: '加害側になるリスク。CI/CDの認証や鍵管理は厳格に。' },
      { head: '使っているライブラリの一覧(SBOM)を作っていない:', body: '新しい弱点が見つかった時に「自社が影響あるか」を素早く判定できません。' }
    ],
    caseStudy:
