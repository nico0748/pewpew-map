...
export const jailbreak: AttackDefinition = {
  meta,
  appliesIf: [
    { head: '一般公開しているAIチャットボットを運営:', body: '誰でも入力できる窓口がある時点でJailbreakの試行に晒される。' },
    { head: 'AIに「答えてはいけない」カテゴリを設けている:', body: '禁止事項がある=回避を試される。' },
    { head: 'AIの応答が外部発信(Twitter/SNS)に直結:', body: '不適切応答が即拡散され、ブランド毀損リスクが大きい。' },
    { head: '入出力に分類器(Llama Guard等)を挟んでいない:', body: 'モデル単独の安全訓練だけでは不十分。' },
    { head: 'システムプロンプトの位置/強度を意識せず書いている:', body: '長文会話で制約が薄まる現象に備えていない。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: '誰でも触れるAIチャットボットを公開している:', body: '誰でも入力できる時点で、Jailbreak の試行を絶えず受ける立場になります。' },
      { head: 'AIに「答えてはいけない」カテゴリ設定がある:', body: '禁止項目がある=その回避を試されると考えてください。' },
      { head: 'AIの応答が SNS に投稿される作りになっている:', body: '不適切な応答が一瞬で拡散され、ブランドが傷つきます。' },
      { head: '入出力チェック用の専用フィルタを入れていない:', body: 'AI本体の訓練だけでは限界があり、Llama Guard などの専用フィルタが必要。' },
      { head: '長い会話の途中でシステム指示を再注入していない:', body: '対話が続くにつれて安全制約が薄まる現象があるので、定期的に強い指示を入れ直すべきです。' }
    ],
    caseStudy:
