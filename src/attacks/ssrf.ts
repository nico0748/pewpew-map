...
export const ssrf: AttackDefinition = {
  meta,
  appliesIf: [
    { head: 'URL を受け取って取得する機能がある:', body: 'リンクのプレビュー / OG画像取得 / Webhook 受信 / PDFレンダ / RSS取り込みなど。' },
    { head: 'クラウド(AWS/GCP/Azure)上で動いている:', body: 'メタデータサービス経由で IAM 一時クレデンシャルを奪われる経路が存在する。' },
    { head: 'IMDSv2強制になっていない(AWSの場合):', body: 'IMDSv1が有効だとSSRFで認証情報が即取られる。' },
    { head: 'アプリプロセスから内部ネットへ通信できる:', body: '169.254 や RFC1918 への接続を遮断していないと SSRF の旨味が大きい。' },
    { head: '外向き HTTP のスキーム/ドメイン制限が無い:', body: '`file://` `gopher://` の混入や任意ドメインへの送信が可能。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: '「URLを受け取ってサーバが取りに行く」機能がある:', body: 'リンクプレビュー、OG画像の取得、Webhook受信、URLからのPDF生成、RSS取り込みなどはすべて該当します。' },
      { head: 'AWS / GCP / Azure などクラウド上で動いている:', body: 'クラウド独自の「内部メタデータ受付」アドレスを呼び出されて、認証情報を抜かれる経路があります。' },
      { head: 'AWSでIMDSv2を必須にしていない:', body: '古いIMDSv1のままだと、SSRF経由で一時的な認証キーが即抜かれます。' },
      { head: 'アプリから社内ネットワークに通信できる構成:', body: '社内専用サーバや `169.254.x.x` への通信を遮断していないと、SSRFが社内荒らしにつながります。' },
      { head: '外向きHTTPに「許可ドメイン」「許可スキーム」を設定していない:', body: '`file://` で内部ファイルを読まれたり、任意のドメインへリクエストを飛ばされたりします。' }
    ],
    caseStudy:
