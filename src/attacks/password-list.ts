...
export const passwordList: AttackDefinition = {
  meta,
  appliesIf: [
    { head: 'パスワードログインを採用している:', body: '利用者がパスワードを使い回す前提で、リスト型攻撃の影響を受ける可能性がある。' },
    { head: 'メールアドレスをログインIDにしている:', body: 'メアド+PW の組み合わせは他社流出リストとの突合に最も使われる形式。' },
    { head: 'MFAが任意設定になっている:', body: '"設定する利用者だけ守られる" 作りは構造的に被害を受け続ける。' },
    { head: 'ログイン成功通知が無い:', body: '本人が異常に気付ける手段(メール通知)が無いと検知が遅れる。' },
    { head: '初回端末/未知ASN/海外接続でも追加認証なし:', body: 'リスクベース認証が無いと攻撃が通りやすい。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: 'パスワードログインを採用している:', body: '利用者がパスワードを使い回している前提では、ほぼ全サービスに影響が及びます。' },
      { head: 'メールアドレスをログインIDに使っている:', body: '「メールアドレス + パスワード」の組み合わせが最もよく流出リストに登場するパターンです。' },
      { head: '多要素認証(MFA)を必須にしていない:', body: '「設定した人だけ守られる」状態だと、全体としては守りきれません。' },
      { head: 'ログインがあったときに本人通知をしていない:', body: '見覚えのないログインを本人がすぐ気付ける仕組みがないと、被害が拡大します。' },
      { head: '初めての端末や海外からのログインでも追加確認をしない:', body: 'いつもと違う使われ方への警戒がないと、攻撃が通りやすい。' }
    ],
    caseStudy:
