...
export const ddos: AttackDefinition = {
  meta,
  appliesIf: [
    { head: 'インターネットに公開している全サービス:', body: '公開している以上、誰でも攻撃対象になり得る。' },
    { head: '単一サーバ/単一リージョン構成:', body: '冗長化していないと帯域が一拠点に集中するため落ちやすい。' },
    { head: '重い計算を伴うエンドポイントがある:', body: '画像変換・全文検索・PDF生成等、1リクエストの負荷が高いものは攻撃の旨味が大きい。' },
    { head: 'CDNやWAFを通していない:', body: '直接オリジンに到達する構成は被弾を一身に受ける。' },
    { head: '従量課金のクラウドで動いている:', body: 'サービス停止は防げても料金が膨らむ "経済的DDoS" を受ける。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: 'ネットに公開しているサービスを運営している:', body: '公開している以上、誰でも攻撃の標的になりうるので「自分には関係ない」とは思わないこと。' },
      { head: 'サーバが1台/1拠点しかない:', body: '冗長化(複数台に分散)していないと、攻撃が来た時に一気に落ちます。' },
      { head: '画像変換や全文検索など重い機能を提供している:', body: '1リクエストで負荷がかかる作りは、攻撃者にとって少ないリクエストで沈められる旨味があります。' },
      { head: 'CloudflareやWAFのような前段サービスを入れていない:', body: '攻撃を直接サーバが浴びる構成は被害が大きい。' },
      { head: 'クラウドの「使った分だけ課金」プランを使っている:', body: 'サービスを止められなくても、攻撃で通信量が爆発して料金請求が跳ね上がります(経済的DDoS)。' }
    ],
    caseStudy:
