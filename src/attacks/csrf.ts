...
export const csrf: AttackDefinition = {
  meta,
  appliesIf: [
    { head: 'Cookie/セッションでログインを保持している:', body: 'ブラウザがリクエストに自動付与するCookie認証はCSRFの前提条件。' },
    { head: '送金/退会/権限変更などの状態変更APIがある:', body: 'クリック/フォーム送信1発で実行される重要操作はCSRFの最高難度のターゲット。' },
    { head: 'GETで状態変更している箇所がある:', body: 'リンク踏ませただけで変わる作りは設計上アウト。' },
    { head: 'CORSで守れていると思い込んでいる:', body: 'CORSはJSのfetch等が対象。フォームsubmitは制限されないため別途対策が必要。' },
    { head: 'JSON APIでも Content-Type 検証していない:', body: 'text/plain や multipart で送られると "シンプルリクエスト" 扱いで届く。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: 'クッキーでログイン状態を保持している:', body: '一度ログインしたら次回も自動でログイン状態になるタイプ(=ほぼ全てのWebサービス)。' },
      { head: '送金・退会・パスワード変更のような大事な操作がボタン1発:', body: '追加確認なしで重要操作が完了する作りは、CSRFの直撃を受けます。' },
      { head: 'リンクを踏むだけで何かが変わる箇所がある:', body: '`/posts/123/delete` のようにGETリクエストで状態を変える作りは設計から見直しが必要。' },
      { head: 'JSON APIにも対策をしていない:', body: '「JSONだから安全」は誤解。Content-Typeを工夫されると外部サイトから送られます。' },
      { head: '管理画面が普通のブラウザで使える:', body: '管理者がログイン中に罠ページを踏むと、利用者全員に影響する操作を勝手にされます。' }
    ],
    caseStudy:
