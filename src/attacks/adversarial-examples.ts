...
export const adversarialExamples: AttackDefinition = {
  meta,
  appliesIf: [
    { head: '画像分類/物体検出/OCRをアプリで使っている:', body: '画像を入力に判定するモデルは敵対的サンプルの主要ターゲット。' },
    { head: 'スパム/不正/ボット検知 等のセキュリティ判定にAI:', body: '回避を最適化されると検知をすり抜けられる。' },
    { head: '顔認証/音声認証で本人確認を行う:', body: '生体認証回避でなりすましが成立する。' },
    { head: 'モデルAPIで logit / 確率分布を返している:', body: '攻撃者の摂動最適化を効率化させる情報を渡すことになる。' },
    { head: 'NLP系の判定器を使っている:', body: '同義語置換 / Unicode類似文字 / 誤字混入で誤分類する。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: '画像を判定するAIをアプリで使っている:', body: '画像分類・物体検出・OCRなど、画像を入力にしてAIで判定するシステムは主要なターゲットです。' },
      { head: 'スパム判定や不正検知にAIを使っている:', body: '攻撃者が回避を最適化してくる相手なので、すり抜けられるリスクがあります。' },
      { head: '顔認証や音声認証で本人確認をしている:', body: 'AI判定を破られるとなりすましが成立してしまいます。' },
      { head: 'AI APIで生のスコアや確率を返している:', body: '攻撃者に「どっちに近いか」のヒントを渡すことになり、細工がしやすくなります。' },
      { head: '文章を判定するAIを使っている:', body: '同じ意味の言い換え、見た目が似ている文字、わざとの誤字でも誤判定を引き起こせます。' }
    ],
    caseStudy:
