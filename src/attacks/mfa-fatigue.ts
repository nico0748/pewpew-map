...
export const mfaFatigue: AttackDefinition = {
  meta,
  appliesIf: [
    { head: 'プッシュ通知型MFAを採用している:', body: 'スマホで「承認/拒否」だけで通る作りは MFA 疲労の直撃を受ける。' },
    { head: '番号マッチング(Number Matching)未対応:', body: 'プッシュ通知に確認番号を出してユーザに入力させる仕組みが無いと、誤承認が起こりやすい。' },
    { head: 'プッシュ送出回数の上限が無い:', body: '攻撃者の連打を技術的に止められない。' },
    { head: 'IDP(Okta/Azure AD等)に条件付きアクセスを設定していない:', body: '不審IP/未登録端末からのMFA承認も同じ重みで通ってしまう。' },
    { head: 'IT部門詐称電話/Slackの運用ルールが無い:', body: '社会工学的な誘導と組み合わされて誤承認に至る。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: 'スマホへのプッシュ通知で MFA を承認している:', body: '「OK」「拒否」だけで通る作りは、誤承認の被害を受けやすいです。' },
      { head: '番号合わせ(Number Matching)を入れていない:', body: 'プッシュ通知に2桁の数字を表示し、PCの画面と照合する仕組みが無いと、ボタン1発で通ってしまいます。' },
      { head: '短時間に何度もプッシュを送れる仕様:', body: '攻撃者の連打を技術的に止める仕組みが無いと、利用者が誤承認するまで送り続けられます。' },
      { head: 'ID基盤(Okta / Microsoft Entra ID 等)で条件付きアクセスを設定していない:', body: '見慣れないIPや端末からのログインにも、ふだんと同じMFAだけで通ってしまいます。' },
      { head: '「IT部門を名乗る連絡」の運用ルールが無い:', body: '電話やSlackで「テストなので承認してください」と言われて従ってしまう余地があります。' }
    ],
    caseStudy:
