...
export const xss: AttackDefinition = {
  meta,
  appliesIf: [
    { head: 'ユーザの投稿/コメント/プロフィールを画面に出している:', body: '入力をそのままHTMLとして出している箇所はXSSの典型的な攻撃面。' },
    { head: 'リッチテキスト/Markdown対応:', body: '太字や画像など装飾を許可する設計は、サニタイズを怠るとほぼ確実に穴ができる。' },
    { head: 'innerHTML / dangerouslySetInnerHTML / v-html を使っている:', body: '"安全なAPIだけ使う" 原則を破る記述。レビューの最重要観点。' },
    { head: 'URLをユーザから受け取り a/img/iframe に出している:', body: 'javascript: や data: スキームの混入で XSS に化ける。' },
    { head: 'URLパラメータを画面に表示している:', body: '反射型XSSの温床(エラー文/検索ワード再表示など)。' }
  ],
  caseStudy:
  ...
  beginner: {
    ...
    appliesIf: [
      { head: '人が書き込んだ内容を他の人に見せる機能がある:', body: '掲示板・SNS投稿・レビュー・コメント・プロフィール表示など、利用者の入力を画面に出している部分はすべて要注意。' },
      { head: '太字や画像が貼れる「リッチな入力欄」がある:', body: 'ブログのように装飾を許す設計は、対策をしないとほぼ確実に穴ができます。' },
      { head: '`innerHTML` / `dangerouslySetInnerHTML` / `v-html` を使っている:', body: '「HTMLをそのまま流し込む書き方」をしている箇所は、その時点で危険です。' },
      { head: 'URLを利用者に入力させて画面に表示している:', body: 'プロフィールのリンクや、画像アップロード後のURL表示など。' },
      { head: 'URLの ?キーワード= を画面に出している:', body: '検索結果ページなどで「検索キーワード: ○○」と再表示している場合も対象です。' }
    ],
    caseStudy:
